/* ==================================================
   ...
   "fichas": arrastre con el BOTÓN IZQUIERDO (mouse; touch
   no distingue botones, así que ahí sigue siendo
   cualquier arrastre) sobre su mesh lo gira alrededor de
   su propio centroide — arrastre HORIZONTAL gira sobre el
   eje vertical (yaw, como antes), arrastre VERTICAL gira
   sobre el eje horizontal "de frente" del objeto (pitch,
   nuevo) — sin tocar la escena ni al resto de los
   elementos del círculo.

   BOTÓN IZQUIERDO, NO CUALQUIERA: el derecho ahora es
   paneo de cámara (ver galeria-paneo.js, mismo
   renderer.domElement, filtra el suyo en su propio
   pointerdown) — antes de que existiera paneo, este
   módulo no filtraba por botón y el derecho también
   rotaba, efecto colateral no buscado.
...
export function createInteraccionFicha(
    config,
    {
        ...
        /*
            Mallas ADICIONALES, aparte de
            cones[elementoActivoId].userData.mallas, contra
            las que también vale arrastrar — hoy, los planos
            de corte visibles (ver galeria-plano-corte.js,
            obtenerMallasHitTest): técnicamente cuelgan del
            mismo "mallaFrontal" del elemento en foco, misma
            subescena, así que agarrar el plano para rotar es
            tan válido como agarrar la geometría real. Se
            llama en cada hit-test (pointerdown), no se
            cachea por frame, para no arrastrar un estado
            viejo si el switch se prendió/apagó entre un
            frame y el click. Opcional: por defecto, ninguna
            malla extra — mismo criterio que
            getManualOffset = () => ({ yaw: 0, pitch: 0 }) en
            galeria-carrusel.js para "nadie pasó nada".
        */
        obtenerMallasExtra = () => []
    }
) {
    ...
    */
    ...
    function hitTestElementoActivo(clientX, clientY) {
        ...
        /*
            Geometría real + planos de corte visibles (ver
            "obtenerMallasExtra" en la firma de arriba) — no
            rompe el criterio "SÓLO el elemento en foco es
            arrastrable" de la cabecera: los planos que
            devuelve obtenerMallasExtra() son, por
            construcción, siempre los del MISMO elemento en
            foco (galeria-corte.js/galeria-plano-corte.js se
            mantienen sincronizados con el mismo
            "elementoId" que ya usa este módulo, ver
            galeria.js), nunca los de otro cono del círculo.
        */
        const mallas =
            cones[elementoActivoId].userData.mallas
                .concat(obtenerMallasExtra());
    ...
    function onPointerDown(ev) {
        ...
        /*
            SÓLO botón izquierdo (button === 0): el
            derecho ahora es paneo (ver galeria-paneo.js,
            que escucha el mismo renderer.domElement,
            filtrando el suyo). Antes este handler no
            filtraba por botón, así que un click derecho
            también rotaba — efecto colateral no buscado,
            además de que dejaba el botón derecho pisado
            para lo que de verdad se espera de él
            (paneo, y el menú contextual nativo del
            navegador fuera de "fichas").
        */
