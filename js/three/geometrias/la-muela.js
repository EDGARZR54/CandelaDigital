...
export function crearGeometriaConoSinusoidal(
  ...
  /*
      ANCLAJE A PISO (Y=0), invariante a Z1/Z2.

      A diferencia de otros generadores de esta galería
      (los-manantiales.js, concha-acustica.js), acá la
      altura (zRhino -> Y de Three.js) depende
      directamente de Z1/Z2 sin ningún piso implícito en
      la fórmula: zRhino = (Z1+Z2)/2 + término·(x²-y²)/R².
      Mover el slider de Z1 o Z2 corre el mínimo real de
      Y sin que nada lo compense, y como el Group se
      ...
      Z1/Z2/R/numDivisiones — mismo criterio de "piso
      fijo" que ya cumplen por construcción los otros
      generadores de la galería.
  */
  geometriaFinal.computeBoundingBox();
  geometriaFinal.translate(
    0, -geometriaFinal.boundingBox.min.y, 0
  );
