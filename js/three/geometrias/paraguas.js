...
export function superficieHyparPetalo(uNorm, vNorm, destino, parametros = {}) {
  const {
    ...
    numSectores = PARAMETROS_DEFECTO.numSectores
  } = parametros;

  const t = vNorm;

  // --- Geometría del polígono regular de n lados ---
  // halfAngle = mitad del ángulo de un sector (2π / n).
  // Con n=4, halfAngle = 45°, reproduciendo exactamente el caso original.
  const halfAngle = Math.PI / numSectores;

  // Radio del anillo de las "esquinas altas" P1/P3.
  // Generaliza el R*sqrt(2) original: R / cos(45°) = R*sqrt(2) cuando n=4.
  const Rd1 = R / Math.cos(halfAngle);

  // Radio del anillo de la "esquina exterior baja" P2, un paso más afuera
  // (misma relación aplicada de nuevo, como en el caso n=4: R*sqrt(2)*sqrt(2) = 2R).
  const Rd2 = Rd1 / Math.cos(halfAngle);

  // Ángulos: P1 en una referencia fija (90°), P3 exactamente un sector
  // completo (2π/n) más adelante, y P2 en la bisectriz entre ambos.
  const angle1 = Math.PI / 2;
  const angle3 = angle1 + 2 * halfAngle;
  const angle2 = angle1 + halfAngle;

  // Puntos clave del pétalo (Rhino local, sector 0)
  const P1 = { x: Rd1 * Math.cos(angle1), y: Rd1 * Math.sin(angle1), z: Z1 };
  const P2 = { x: Rd2 * Math.cos(angle2), y: Rd2 * Math.sin(angle2), z: Z2 };
  const P3 = { x: Rd1 * Math.cos(angle3), y: Rd1 * Math.sin(angle3), z: Z1 };
...
export function calcularAlturaColumna(parametros = {}) {
  ...
  // NOTA / LIMITACIÓN: esta aproximación (u = v = anchoColumna / Rd1) asume
  // que el punto de la columna cae en la esquina (w, w) de un cuadrado
  // ortogonal, lo cual sólo es geométricamente exacto para numSectores = 4
  // (donde los ejes x,y locales son simétricos). Para n != 4 sigue dando
  // una altura razonable cerca del centro (P4), pero deja de ser una
  // proyección exacta si anchoColumna es grande en relación a R. Si
  // necesitas precisión total para n != 4, lo correcto es reemplazar la
  // columna por un prisma/cilindro de radio `anchoColumna` centrado en el
  // origen, y resolver (u, v) invirtiendo el loft bilineal para el punto
  // real (radioColumna*cos(angle2), radioColumna*sin(angle2)) en vez de
  // usar esta aproximación por ejes ortogonales.
  const u = anchoColumna / Rd1;
  const v = anchoColumna / Rd1;
